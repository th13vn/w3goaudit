...
contract VulnerableCompoundBorrowFresh {
    mapping(address => uint256) public accountBorrows;

    function borrowFresh(address payable borrower, uint256 amount) external {
        doTransferOut(borrower, amount);
        accountBorrows[borrower] = amount;
    }

    function doTransferOut(address payable borrower, uint256 amount) internal {
