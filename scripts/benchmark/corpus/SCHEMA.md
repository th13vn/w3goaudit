...
`scripts/benchmark/config/<tool>/detectors.json`, not in every corpus. `run_benchmark.py`
...
```json
{
  ...
  "target": "scripts/benchmark/fixtures/slither-detectors/",
  "templates": "scripts/benchmark/templates/slither-inspired",
...
| `semgrep_config` | no | Path to a Semgrep config dir/file for this case (used by the `decurity` and `competitive` suites to point at the vendored Decurity ruleset). Omit to use the run-wide `--semgrep-config` (default: `scripts/benchmark/config/semgrep-decurity`). |
...
1. Create `scripts/benchmark/corpus/<name>.json` following the structure above.
