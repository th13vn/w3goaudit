...
threshold gate all live under `scripts/benchmark/`.
...
Durable reports are stored as tracked dated files in `benchmarks/`
(`yyyy-mm-dd-<commit-slug>.md`, a copy of the run's `benchmark.md`);
`benchmarks/results/` is the Git-ignored run-output scratch directory.

Docker Compose is the only supported host entry point **for the multi-tool
comparison** (Slither/Semgrep/4naly3er). In that lane the host does not need
...
The active suites are fully owned by `scripts/benchmark/corpus/`; the `SUITES` map in
...
```bash
python3 -m compileall -q scripts/benchmark
(cd scripts/benchmark && python3 -m unittest discover -p 'test_*.py' -v)
```

These commands verify the harness itself. The canonical four-tool command
remains:
