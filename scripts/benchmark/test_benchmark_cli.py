...
class ShellContractTest(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for tools, expected_calls in (("w3goaudit", 2), ("slither", 1)):
                with self.subTest(tools=tools):
                    ...
                    self.assertIn("scripts/benchmark/run_benchmark.py", calls[0])
                    if tools == "w3goaudit":
                        self.assertEqual(
                            calls[1],
                            "scripts/benchmark/assert_thresholds.py "
    ...
    def test_host_workflows_are_local_cli_and_compose(self):
        for stale in (
            "benchmarking.md",
        ...
        self.assertIn("python3 scripts/benchmark/run_benchmark.py", readme)
        self.assertIn("assert_thresholds.py", readme)
        self.assertNotIn("run_docker_benchmark.sh", readme)

        self.assertTrue(
            (ROOT / "benchmarks" / "results" / ".gitkeep").is_file()
        )
        ...
        forbidden = {
            "direct host W3GoAudit benchmark": re.compile(
                r"^\s*(?:\$\s+)?w3goaudit\s+scripts/benchmark/"
            ),
