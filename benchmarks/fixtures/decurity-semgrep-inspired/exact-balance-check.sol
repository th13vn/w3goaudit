...
contract VulnerableExactBalanceCheck {
    function unlock(IERC20Like token, address user, uint256 amount) external view returns (bool) {
        require(token.balanceOf(user) == amount, "exact");
        return true;
    }
}
