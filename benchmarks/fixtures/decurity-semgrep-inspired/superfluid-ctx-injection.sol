...
contract VulnerableSuperfluidCtxInjection {
    function decode(ISuperTokenLike token, bytes calldata ctx) external view returns (address app, address sender) {
        return token.decodeCtx(ctx);
    }
}
